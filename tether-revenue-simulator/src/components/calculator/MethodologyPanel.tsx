    ...
    <Card className="mt-8">
      ...
      >
        <div>
          ...
          <p className="text-sm text-brand-muted">
            How we calculate your revenue estimates
          </p>
        </div>
        {/* Arrow points down when closed, rotates up when open — content always opens downward */}
        <svg
      ...
      {isExpanded && (
        <div className="mt-6 pt-6 border-t border-brand-border space-y-6">
          {/* E-Credits */}
          <div>
            <h4 className="text-sm font-semibold text-brand-ecredit mb-2">
              E-Credit Revenue
            </h4>
            <div className="bg-brand-surface rounded-lg p-4 text-sm text-brand-muted space-y-2">
              <p>
                <strong className="text-brand-text">Annual kWh</strong> = Chargers x Power(kW) x
                Utilization x Accessible Hours x 365
              </p>
              <p>
                <strong className="text-brand-text">Effective Rate</strong> = Renewable Energy % x
                Multiplier(4) x Avoided CO2(0.3234 kg) x CO2 Price(0.34
                EUR/kg) x Market Discount(3.5%)
              </p>
              <p>
                <strong className="text-brand-text">CPO E-Credit Revenue</strong> = Annual kWh x Effective
          ...
          {/* Grid Flexibility */}
          <div>
            <h4 className="text-sm font-semibold text-brand-warm mb-2">
              Grid Flexibility Revenue
            </h4>
            <div className="bg-brand-surface rounded-lg p-4 text-sm text-brand-muted space-y-2">
              <p>
                <strong className="text-brand-text">Available Capacity</strong> = Chargers x Utilization x
                Power(MW) x Flex Potential
              </p>
              <p>
                <strong className="text-brand-text">Monthly Flex Revenue</strong> = Sum of (mFRR Up + mFRR
                Down + FCR-D Up + FCR-D Down) x Accessible Hours x CPO
                Share(40%)
              </p>
              <p>
                Market prices vary by month and country, based on historical auction
